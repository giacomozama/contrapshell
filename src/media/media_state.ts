...
import { MediaState as MediaPlayerState, MediaStatus } from "./types";
...
export type MediaState = {
    ...
    availableMediaPlayers: Accessor<AstalMpris.Player[]>;
    activeMediaPlayer: Accessor<AstalMpris.Player | undefined>;
    watchActivePlayer(player: AstalMpris.Player | undefined): void;
};

let mediaStateInstance: MediaState | undefined;

// needs to be top-level or it throws errors. don't know why.
let lastPalettePath = "";
...
function createMediaState(): MediaState {
    ...
    const availableMediaPlayers = createComputed(() =>
        mediaPlayers().filter((p) =>
            createBinding(p, "playbackStatus").as((s) => s !== AstalMpris.PlaybackStatus.STOPPED)()
        )
    );
    ...
    const [mediaPalette, setMediaPalette] = createState<Gdk.RGBA[] | null>(null);
    ...
    const mediaPreviewLabelText = createComputed(() => {
    ...
    function mediaCycleLoop() {
        const player = activeMediaPlayer?.peek();
        if (!player) return;
    ...
    const playerState: MediaPlayerState = {
        status: mediaStatus,
        ...
        playPause: () => activeMediaPlayer?.peek()?.play_pause(),
        skipPrevious: () => activeMediaPlayer?.peek()?.previous(),
        skipNext: () => activeMediaPlayer?.peek()?.next(),
        stop: () => activeMediaPlayer?.peek()?.stop(),
        cycleLoop: mediaCycleLoop,
        cycleShuffle: mediaCycleShuffle,
        seek: (position) => activeMediaPlayer?.peek()?.set_position(position),
    };
    ...
    function watchActivePlayer(player: AstalMpris.Player | undefined) {
        if (player?.busName === activeMediaPlayer.peek()?.busName) return;
    ...
    function getIndexOfFirstPlayingPlayer() {
        const players = availableMediaPlayers.peek();
    ...
    createEffect(() => {
        const players = availableMediaPlayers();
        ...
        for (const player of players) {
            function checkStatus() {
                if (activeMediaPlayer.peek()?.busName === player.busName) {
                    if (player.playbackStatus !== AstalMpris.PlaybackStatus.PLAYING) {
                    ...
                    activeMediaPlayer.peek()?.playbackStatus !== AstalMpris.PlaybackStatus.PLAYING &&
    ...
    watchActivePlayer(availableMediaPlayers.peek()[firstPlayingPlayerIndex ?? 0]);
    ...
    function updateBorderColor(artUrl: string) {
        ...
        function onFileReady(path: string) {
            generatePalette(path, 2, "colorful", "snic").then((palette) => {
    ...
    updateBorderColor(mediaArtPath.peek());
...
export function mediaState(): MediaState {
    return mediaStateInstance ?? createRoot(createMediaState);
}
