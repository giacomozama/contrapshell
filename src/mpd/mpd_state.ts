...
export type MpdState = {
    mpdCurrentSongFile: Accessor<string>;
    mpdPlaybackStatus: Accessor<AstalMpris.PlaybackStatus>;
    mpdMusicLibrary: Accessor<Album[]>;
    mpdPlayAlbum: (album: Album, startingFrom: number) => void;
    mpdShufflePlayAll: () => void;
};

let mpdStateInstance: MpdState | null = null;

function createMpdState(): MpdState {
    ...
    (async () => {
        ...
        const cacheIndex: { [key: string]: string } = indexFile.query_exists(null)
            ? JSON.parse(readFile(indexFile))
            : {};
    ...
    const [mpdCurrentSongFile, setCurrentSongFile] = createState("");
    const [mpdPlaybackStatus, setMpdPlaybackStatus] = createState(AstalMpris.PlaybackStatus.STOPPED);
    ...
    function updatePlaybackStatus() {
        setMpdPlaybackStatus(mpdPlayer.peek()?.playback_status ?? AstalMpris.PlaybackStatus.STOPPED);
    }

    let disconnectPlayerTitle: (() => void) | undefined;
    let disconnectPlayerStatus: (() => void) | undefined;

    createEffect(() => {
        disconnectPlayerTitle?.();
        disconnectPlayerTitle = undefined;
        disconnectPlayerStatus?.();
        disconnectPlayerStatus = undefined;

        const player = mpdPlayer();
        if (!player) return;

        const titleConnId = player.connect("notify::title", updateCurrentSong);
        disconnectPlayerTitle = () => player.disconnect(titleConnId);
        updateCurrentSong();

        const statusConnId = player.connect("notify::playback-status", updatePlaybackStatus);
        disconnectPlayerStatus = () => player.disconnect(statusConnId);
    ...
    return mpdStateInstance;
}

export function mpdState() {
    return mpdStateInstance ?? createRoot(createMpdState);
}
