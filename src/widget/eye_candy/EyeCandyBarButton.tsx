...
import { EyeCandyMode } from "../../state/eye_candy/types";
import { eyeCandyConfig, setEyeCandyMode } from "../../state/eye_candy/eye_candy_state";
...
function EyeCandyPopover() {
    const optionButton = (mode: EyeCandyMode, iconName: string, title: string, description: string) => {
        ...
        return (
            <togglebutton
                class={"popover-button-list-item"}
                cursor={CURSOR_POINTER}
                onClicked={() => setEyeCandyMode(mode)}
                active={active}
            >
                <box>
                    <image class="popover-button-list-item-icon" valign={Gtk.Align.CENTER} iconName={iconName} />
    ...
    return (
        <glassypopover widthRequest={500}>
            <box orientation={Gtk.Orientation.VERTICAL} cssClasses={["popover-standard-inner"]}>
                ...
                <box orientation={Gtk.Orientation.VERTICAL} class={"popover-button-list"}>
                    {optionButton(
                        EyeCandyMode.Off,
