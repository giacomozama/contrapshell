...
import { Album, Song } from "../../state/mpd/types";
import { CURSOR_POINTER } from "../../utils/gtk";
import { Accessor, createState, For, onCleanup } from "gnim";
import Pango from "gi://Pango?version=1.0";
import { currentMpdSongFile, mpdMusicLibrary, mpdPlayAlbum, mpdShufflePlayAll } from "../../state/mpd/mpd_state";
...
function SongList({
    ...
    return (
        <scrolledwindow $type="named" name="album" vexpand={true}>
            <box class="popover-button-list" orientation={Gtk.Orientation.VERTICAL} valign={Gtk.Align.START}>
                <button
                ...
                >
                    <box>
                        <image class="go-back-button-icon" iconName="go-previous-symbolic" marginEnd={12} />
                        <label label={"Go back"} />
                    </box>
                </button>
                <box cssClasses={["mpd-album-title-box"]} valign={Gtk.Align.START}>
                    <box>
                        <box
                    ...
                    {(song: Song, index) => (
                        <button
                            cssClasses={currentMpdSongFile.as((f) =>
                                f === song.file
                                    ? ["mpd-song-button", "popover-button-list-item", "current"]
                                    : ["mpd-song-button", "popover-button-list-item"]
                            )}
                            cursor={CURSOR_POINTER}
                            valign={Gtk.Align.START}
